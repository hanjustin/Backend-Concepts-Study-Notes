...
* **OAuth:** Industry-standard authorization protocol for gaining access to user's resources with limited scopes on third-party application. Allows developers to use an access token as a valet key to gain delegated access without knowing user's identity nor user's credentials.
    ...
    * **Flows:**
        ...
        * **PKCE Flow (Proof Key for Code Exchange):** Enhanced security version of authorization code flow. Additional secret key ("code verifier") is used to guarantee that the system redeeming an authorization code is the same one that requested it. The request for authorization code will contain a hashed value of the secret key and info about used hashing algorithm. Then, the request for redeeming the authorization code will contain the secret key for the authorization server to verify by comparing the received hashed value and generated hashed value from the received secret key.
            * The code verifier needs to be a cryptographically random string that gets dynamically generated by the client as native mobile apps and SPAs are unable to securely store static client secrets.

---
# Passkey
* Passwordless authentication using public-key cryptography.
* Private key is stored on the user's device and usually accessed using local biometric verification.
* Public key is stored on the server. For login, a random challenge from the server gets signed using the private key, and the public key is used to verify the signature.
* Responsibility of security mostly delegated to the local authenticator and cloud syncing services when accessing the private key.
* To login on new devices, the passkey needs to be synced across other devices with a password manager or cloud service.
* Phishing-proof as passkeys are tied to the specific website's domain, and nothing for hackers to steal even when the server gets breached.

---
# Digital Certificate

* **CA (Certificate Authority):** A trusted third party like a notary public for validating a digital certificate.
* **PKI (Public Key Infrastructure):** A system used to create and manage digital certificates.
